...
#define true 1
/*
The following part is some pre-packed code,which helps to deal with 
return values safely. The following are copyed from csapp.c 
*/
...
/*
The above part is some pre-packed code,which helps to deal with 
return values safely. The above are copyed from csapp.c 
*/
 ...
 * 
 * until the progress stops, or the progress is killed by some signal.
*/
void waitfg(void){
    sigset_t mask_emp;
...
/*
 * IOred - IO redirection
 *
 * if tok.infile is not NULL,redirect the input file of correct program to
 * file name tok.infile by closing the default input file.
 * The same happened in tok.outfile and stdout.
*/
void IOredirect(struct cmdline_tokens tok){
    if (tok.infile!=NULL){
        ...
        //assert(fid==STDIN_FILENO);
    }
    if (tok.outfile!=NULL){
        //close(STDOUT_FILENO);
        int fid=Open(tok.outfile,O_WRONLY);
        dup2(fid,STDOUT_FILENO);
        //assert(fid==STDOUT_FILENO);
    }
}
/*
 * Getjob - get jobid from cmdline tok
 *
 * read the jobid which want to do with in tok, and report error 
 * if the argument is lost, or it is not given in the correct way.
 * If the jobid is correct, return the pointer of job_t
 * if is_neg is NULL, then the input %jid/pid should be positive, otherwise
 * is_neg will save whether %jid/pid is negative or not.
*/
        ...
        case BUILTIN_NOHUP:
            ...
            if ((pid = Fork()) == 0){
                ...
                if (tok.builtins==BUILTIN_NONE){
                    /*Not a builtin command, should not ignore argument [0]*/
                ...
                if (tok.builtins==BUILTIN_NOHUP){
                    /* NOHUP command, block SIGHUP, 
                       and ignore the [0] argument */
...
sigchld_handler(int sig) 
{
    ...
    while ((pid=waitpid(-1,&status,WNOHANG|WUNTRACED))>0){
        if (WIFEXITED(status)){
            /*Quit normally,delete pid in job_list*/
            Sigprocmask(SIG_BLOCK,&mask_all,&prev_one);
            deletejob(job_list,pid);
            Sigprocmask(SIG_SETMASK,&prev_one,NULL);
        }
        else if (WIFSIGNALED(status)){
            /*Quit Becaused of a signal,delete pid in job_list*/
            Sigprocmask(SIG_BLOCK,&mask_all,&prev_one);
            ...
            /*Stopped Becaused of a signal,set node to ST(STOP)*/
            Sigprocmask(SIG_BLOCK,&mask_all,&prev_one);
