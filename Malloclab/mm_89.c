...
// give memory a, return pointer of previous block
#define VAL(a)  (*((void**)(a)))
// give pointer a, get the value saved in a in pointer mode
#define NEXTRP(a) (a)
// give empty block b, return pointer of previous block in the chain
#define PREVRP(a) (((char*)(a))+DSIZE)
// give empty block b, return pointer of next block in the chain
 ...
 * Mapping Rule: 1 -> None          2 -> {1~16} + 8      3 -> {17~32} + 8
 *               4 -> {33~64} + 8   5 -> {65~128} + 8    6 -> {129~256} + 8
 *               7 -> {257~512} + 8 8 -> {513~1024} + 8  9 -> {1025~2048} + 8
 *               10 -> {2049~} + 8 
 */
...
void delete_block(void* bp){
    ...
    //fprintf(stderr,"delete_block,pointer = %lx, pre = %lx, nxt = %lx\n",bp, pre, nxt);
    if (nxt != NULL)
        PUTD(PREVRP(nxt),pre);
    if (pre != NULL)
        PUTD(NEXTRP(pre),nxt);
...
void add_block(void* bp){
    int idx = Index(SIZE(HEADRP(bp)));
    void* pre = (char*) heap_p + idx * DSIZE;  // location of head pointer
    void* nxt = VAL(pre);                      // the first element of chain
    //fprintf(stderr,"add_block,pointer = %lx, pre = %lx, nxt = %lx, siz = %x\n",bp, pre, nxt, SIZE(HEADRP(bp)));
    PUTD(PREVRP(bp),pre);
    PUTD(NEXTRP(bp),nxt);
...
void* first_fit(size_t Dnum){
    ...
    while (idx <= 10){
        void* bp = VAL((char*)heap_p + idx * DSIZE);
        //fprintf(stderr,"In firstfit,bpointer = %lx, headpointer = %lx, idx = %d, siz = %x\n",bp,(char*)(heap_p) + idx * DSIZE,idx,Dnum * DSIZE);
        while (bp != NULL){
            size_t sz = SIZE(HEADRP(bp));
            //fprintf(stderr,"bpointer = %lx, bsz = %lx,idx = %d, siz = %x\n",bp,sz,idx,Dnum * DSIZE);
 ...
 */
int mm_init(void){
    heap_p = mem_sbrk(11 * DSIZE + 4 * WSIZE);
    if (heap_p == (void*)-1)
        return -1;
    PUTD(heap_p + 0 * DSIZE, 0);
    PUTD(heap_p + 1 * DSIZE, NULL);
    PUTD(heap_p + 2 * DSIZE, NULL);
    PUTD(heap_p + 3 * DSIZE, NULL);
    PUTD(heap_p + 4 * DSIZE, NULL);
    PUTD(heap_p + 5 * DSIZE, NULL);
    PUTD(heap_p + 6 * DSIZE, NULL);
    PUTD(heap_p + 7 * DSIZE, NULL);
    PUTD(heap_p + 8 * DSIZE, NULL);
    PUTD(heap_p + 9 * DSIZE, NULL);
    PUTD(heap_p + 10 * DSIZE, NULL);
    PUTW(heap_p + 11 * DSIZE + 1 * WSIZE, PACK(DSIZE,1));
    PUTW(heap_p + 11 * DSIZE + 2 * WSIZE, PACK(DSIZE,1));
    PUTW(heap_p + 11 * DSIZE + 3 * WSIZE, PACK(0,1));
...
void *realloc(void *oldptr, size_t sz) {
    ...
    //fprintf(stderr,"Realloc Ins pointer = %lx, size_t = %x, new_sz = %x, Dnum = %x\n",oldptr,sz,newsz,Dnum);
